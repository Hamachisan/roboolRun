...
void PhongShader::Begin(ID3D11DeviceContext* dc, const RenderContext& rc)
{
	...
	//ライトの方向と色の情報をまとめている
	cbScene.directionalLightData = rc.directionalLight;
	//カメラの位置
	...
	for (int i = 0; i < 8; i++)
	{
		cbScene.pointlight[i].position = (rc.pointLight[i].position);
		cbScene.pointlight[i].color = (rc.pointLight[i].color);
		cbScene.pointlight[i].range = (rc.pointLight[i].range);
		cbScene.pointlight[i].dummy = (rc.pointLight[i].dummy);
	}
...
void PhongShader::Draw(ID3D11DeviceContext* dc, const Model* model)
{
	...
	for (const ModelResource::Mesh& mesh : resource->GetMeshes())
	{
		...
		for (const ModelResource::Subset& subset : mesh.subsets)
		{
			...
			dc->PSSetShaderResources(1, 1, subset.material->normal_map.GetAddressOf());
			dc->PSSetSamplers(0, 1, samplerState.GetAddressOf());
