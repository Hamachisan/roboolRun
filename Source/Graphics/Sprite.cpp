...
// SpriteShaderを使うコンストラクタ
Sprite::Sprite(bool spriteShader)
{
	ID3D11Device* device = Graphics::Instance().GetDevice();
...
//フォント
void Sprite::Textout(ID3D11DeviceContext* immediate_context, std::string word,
	...
	float r, float g, float b, float a)
{
	...
	for (const char c : word)
	{
		...
		Render(immediate_context,					//デバイスコンテキスト
			x + carriage, y,						//スクリーン座標 (dx左上, dy左上(画面の左上)
			w, h,									//スクリーン座標 	 dw,横幅  dh　縦幅		
			sw * (c & 0x0F), sh * (c >> 4),		//テクスチャ座標 sx左上, sy左上(画像の左上)
			sw, sh,									//テクスチャ座標 sw横幅, sh縦幅
			(0), r, g, b, a							//角度、rgbaカラー
		);
...
//フォント(数字 int)
void Sprite::TextoutNumInt(ID3D11DeviceContext* immediate_context, int numberInt,
	float x, float y, float w, float h,
	float r, float g, float b, float a)
{
	std::ostringstream oss;
	oss << std::fixed << std::setprecision(1) << numberInt;
	Textout(immediate_context, oss.str(), x, y, w, h, r, g, b, a);
}

//フォント(数字 float)
void Sprite::TextoutNumFloat(ID3D11DeviceContext* immediate_context, float numberFloat,
	float x, float y, float w, float h,
	float r, float g, float b, float a)
{
	std::ostringstream oss;
	oss << std::fixed << std::setprecision(1) << numberFloat;
	Textout(immediate_context, oss.str(), x, y, w, h, r, g, b, a);
}
