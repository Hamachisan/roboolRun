...
ColorGradingShader::ColorGradingShader(ID3D11Device* device)
{
	// 頂点シェーダー
	{
		// ファイルを開く
		FILE* fp = nullptr;
		fopen_s(&fp, "Shader\\ColorGradingVS.cso", "rb");
	...
	// ピクセルシェーダー
	{
		// ファイルを開く
		FILE* fp = nullptr;
		fopen_s(&fp, "Shader\\ColorGradingPS.cso", "rb");
		_ASSERT_EXPR_A(fp, "CSO File not found");
	...
	// 定数バッファ
	{
		...
		HRESULT hr = device->CreateBuffer(&desc, 0, colorGradingConstantBuffer.GetAddressOf());
		_ASSERT_EXPR(SUCCEEDED(hr), HRTrace(hr));
	}
