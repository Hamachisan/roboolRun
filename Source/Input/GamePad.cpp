...
void GamePad::Update()
{
	...
	if (XInputGetState(slot, &xinputState) == ERROR_SUCCESS)
	{
		...
		if (pad.wButtons & XINPUT_GAMEPAD_B)						newButtonState |= BTN_PAD_B;
		if (pad.wButtons & XINPUT_GAMEPAD_X)						newButtonState |= BTN_PAD_X;
		if (pad.wButtons & XINPUT_GAMEPAD_Y)						newButtonState |= BTN_PAD_Y;
		if (pad.wButtons & XINPUT_GAMEPAD_START)					newButtonState |= BTN_PAD_MENU;
	...
	// キーボードでエミュレーション
	{
		...
		//j,k,lキー
		if (GetAsyncKeyState('J') & 0x8000) newButtonState |= BTN_J;
		if (GetAsyncKeyState('K') & 0x8000) newButtonState |= BTN_K;
		if (GetAsyncKeyState('L') & 0x8000) newButtonState |= BTN_L;
